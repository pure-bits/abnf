...
lazy val sharedSettings = Seq(
  projectTitle := "abnf",
  projectWebsiteRootURL := "https://purebits.github.io/",
  projectWebsiteBasePath := "/abnf/",
  githubOwnerID := "zakolenko",
  githubRelativeRepositoryID := "abnf",

  organization := "io.github.purebits",
  scalaVersion := "2.13.6",
...
lazy val root = project.in(file("."))
  ...
  .aggregate(coreJVM, coreJS, algebraJVM, algebraJS)
  .configure(defaultPlugins)
  .settings(sharedSettings)
  .settings(doNotPublishArtifact)
  .settings(unidocSettings(coreJVM, algebraJVM))
  .settings(
...
lazy val site = project.in(file("site"))
  ...
  .settings {
    ...
    Seq(
      ...
      micrositeDescription := "abnf",
      micrositeAuthor := "purebits",
...
lazy val algebra = crossProject(JSPlatform, JVMPlatform)
  ...
  .settings(
    ...
    libraryDependencies ++= Seq(
      "org.typelevel"  %%% "cats-core"        % CatsVersion,
      "org.typelevel"  %%% "cats-effect"      % CatsEffectVersion,
      "io.higherkindness" %%% "droste-macros" % "0.9.0-M3",
      // For testing
      "org.scalatest"     %%% "scalatest"        % ScalaTestVersion % Test,
      "org.scalatestplus" %%% "scalacheck-1-15"  % ScalaTestPlusVersion % Test,
      "org.scalacheck"    %%% "scalacheck"       % ScalaCheckVersion % Test,
